...
- Daily bars only — no intraday data. These shapes are multi-day structures.
  ...
  once the archive has depth. **Amended 2026-09-22:** the archive reached that
  depth — the shared §5 floor opened 2026-09-18 and six campaigns are measured
  (pre-regs #15/#19/#21/#22/#27/#32; verdicts `CLAIMS_LEDGER.md` §K.1–§K.6).
  The track now has two arms: the S&P 600 archive (`data/intraday`) and the
  mover-universe roster archive (`data/intraday_movers`, pre-reg #33 draft).
  Still no order execution and no real money — pre-reg #23's paper loop is a
  *logging* study of the frozen definitions, not a live loop.)
...
- **Survivorship bias:** backtesting on *current* constituents only sees
  ...
  Section 9.) **(Gate executed 2026-08-15/16, pre-reg #13:** the first
  positive F1 result — the RSI-divergence F1-BULL EDGE (pre-reg #10) — was
  re-checked against (a), the historical-constituent union (904 names from
  5 annual S&P 600 snapshots 2021–2025 incl. ~330 delisted). **Gate FAILS:**
  F1-BULL NO EDGE on the corrected window (OOS 2022–2025, n=9,384, p_input
  0.072, CI-low −0.15pp) — the EDGE is corrected to the survivorship-resilient
  record and the family is closed; CLAIMS_LEDGER §I.10.)**

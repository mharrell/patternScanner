...
- [x] Pre-registration #25 — price-band + float drift adjudication — done
  2026-09-01: **EDGE × 4** — the newer $2–20/$1–20 bands and the ≤20M/≤10M
  float caps all separate from their complements; the sub-band table stays
...
- [x] Pre-registration #26 — market-structure calibration — done 2026-09-01:
  ...
  in §J.3.
- [x] Pre-registration #27 (prep) — drafted AND frozen 2026-09-02 with the
  tool implemented (`measure_macd_gate.py`); floors checked via `--floors`
  (10/20 bar-dates, 19,976/2,000 candidates, 582 tickers) — one-shot NOT
  consumed. Fires when the window opens (~Sep 8–9).
- [x] Pre-registration #28 — daily selection follow-ups — done 2026-09-02:
  gap-and-go **FADE** (−0.22pp, p 0.008 — direction inverted on daily
...
| 2n2-14 | [34:51–36:44] | `partial` (pre-reg #24/#25) — float veto **EDGE** (+0.09pp), price band **EDGE** (+0.40pp), RV leg NO EDGE (#24), +10% leg untested; the same-day sub-$2 self-violation stands (§J.2) | `candidate` | Matches §D scanner rows. **Float band drift:** <20M hard here vs GXl-13's "<10M ideal, <20M fine" and D-02's <10M. The sub-$2 exception is a live internal tension with his own price floor. |
...
| 5X_-04 | [04:50–05:03] | `tested` (pre-reg #28, 2026-09-02) — **FADE**: ≥+2% gappers' open→close UNDERPERFORMS flat-open names (−0.22pp, CI −0.36..−0.06, p 0.008, Holm-cleared) — the named strategy's direction is inverted on daily bars (§J.4) | `candidate` | Named setup definition (gap up on news → intraday continuation). Needs intraday bars. |
| 5X_-05 | [11:28–11:44] | `tested` (pre-reg #25, 2026-09-01) — **EDGE on both legs**: $2–20 (+0.40pp) and float ≤20M (+0.09pp); the "2020 rule" conjunction +1.93pp (descriptive, n=106 dates) (§J.2) | `candidate` | New numeric variant of the pillars: $2–$20 price AND <20M float as a paired "2020 rule". Float is **looser** than ultimate-guide D-02's <10M; price matches D-01. Drift data point for a §I.12-style update. |
...
| 5X_-06b | [13:31–13:42] | `tested` (pre-reg #28, 2026-09-02) — **INCONCLUSIVE** (same rank-2/3 population limitation as 3rE-06) (§J.4) | `candidate` | Softer than D-03's "number one leading percentage gainer"; the measured rank-1-vs-2–10 test didn't cover a top-2/3 definition. |
...
| 3rE-05 | [04:43–04:56] | `tested` (pre-reg #28, 2026-09-02) — **NO EDGE** (≥+10% vs 0–2%: +1.04pp, CI +0.06..+1.95, p 0.038 — the claimed direction, NOT Holm-cleared; the third directional whisper) (§J.4) | `candidate` | Momentum-continuation selection claim; related to I-B-07 (but here continuation is the *preference*, within-day) and pre-reg #4 horizon family. |
| 3rE-06 | [06:34–06:42], [07:16–07:24] | `tested` (pre-reg #28, 2026-09-02) — **INCONCLUSIVE**: rank-2/3-vs-≥10 cells constructible on only 72/1,003 OOS days (the rare-mover population #26's C2 predicted); rank-1 itself remains NO EDGE (D-03) (§J.4) | `candidate` | Rank-1 claim measured NO EDGE on daily bars (D-03, rank-1 vs 2–10, +0.00pp p=0.986); this restatement widens to top-2/top-3 — a rank-2/3-vs-rest test would be a fresh cell. |
...
| 3rE-11 | [22:16–22:25] | `tested` (pre-reg #28, 2026-09-02) — **NO EDGE** (half/whole-dollar breakout entry: the daily proximity form shows no cleared edge; intraday form untested) (§J.4) | `candidate` | Half/whole-dollar breakout entry — a distinct entry rule (psychological-level break); testable intraday. |
...
| ZS8-13 | [20:26–20:33] | `tested` (pre-reg #28, 2026-09-02) — **NO EDGE** (same campaign as ZS8-14) (§J.4) | `candidate` | Round-number proximity as an entry-quality variable — testable: does |apex − nearest half/whole dollar| predict breakout follow-through? |
| ZS8-14 | [22:01–22:21] | `tested` (pre-reg #28, 2026-09-02) — **NO EDGE** (just-below vs just-above round level: −0.04pp, CI −0.08..0.00, p 0.056 — direction consistent with the claim, CI touches zero, not Holm-cleared) (§J.4) | `candidate` | **The most precise level-veto in either video**: apex within pennies *below* a round number → elevated false-breakout risk; apex at/above it → preferred. Directly testable on the intraday archive. |
...
| HYo-03 | [27:14–28:24] | `partial` (pre-reg #24/#25) — price band $1–20 **EDGE** (+0.52pp vs >$20); RV leg NO EDGE (#24); float/news/+10% legs untested (§J.2) | `candidate` | Five-filter scanner. Price band here is **$1–20** — vs 4Pc's $2–20 and I-D-01's $2–5/$2–10 drift. Shares <10M ≈ low-float (I-D-06). |
...
| HYo-14 | [48:11–48:43] | `tested` (pre-reg #28, 2026-09-02) — **NO EDGE** (same campaign as ZS8-14; the coordination mechanism for round-number levels is untested) (§J.4) | `candidate` | Round-number (50¢/$1) support/resistance clustering with a coordination mechanism. Testable-daily and intraday; untested. |
...
| 4Pc-11 | [27:06–28:18] | `partial` (pre-reg #24/#25) — price band $2–20 **EDGE** (+0.40pp); RV leg NO EDGE (#24); +10%/gap>2% legs untested; the P&L attribution remains a red flag (§J.1/§J.2) | `red flag` / `candidate` | Metrics-derived parameter set (+10% day, gap >2%, $2–20). **Third price-band variant in the corpus** ($2–5 → $2–10 → $2–20; HYo-03 says $1–20). I-D-01 measured both $2–5 and $2–10 as EDGE; $2–20 and the +10%/+2% gates are unregistered extensions. |
| 4Pc-11 | [32:09–32:28] | `partial` (pre-reg #24/#25) — price band $2–20 **EDGE** (+0.40pp); RV leg NO EDGE (#24); +10%/gap>2% legs untested; the P&L attribution remains a red flag (§J.1/§J.2) | `candidate` | Entry = first candle to a new high after the light-volume pullback — third repetition of the trigger; cross-ref I-B-02, J-B-03. |
...
| afN-11 | [42:17–42:28] | `tested` (pre-reg #25, 2026-09-01) — **EDGE**: the $2–20 band separates from >$20 (+0.40pp) (§J.2) | `candidate` | Price-band criterion with numbers; same 2–20 band repeated in dkO-12; daily-testable. |
...
| dkO-12 | [46:50–47:27] | `partial` (pre-reg #25) — the 2–20 band itself tested **EDGE** (+0.40pp); the self-reported bucket P&L remains unverifiable (§J.2) | `red flag` | Self-reported price-band profitability; the 2–20 band itself is daily-testable and matches afN-11 (three sub-bands given here). |
...
| GMR-07 | [10:31–10:40] | `tested` (pre-reg #28, 2026-09-02) — **NO EDGE** (same campaign as 3rE-05: nominal p 0.038 in the claimed direction, not significant after Holm) (§J.4) | `candidate` | Intraday momentum-continuation claim; testable as conditional P(double-from-10%) vs P(0→10%). Distinct from I-B-07's day-2 continuation question. |
...
## J.2 Price-band + float-cap verdicts — pre-registration #25 campaign (2026-09-01)

The §J scan found the corpus's scanner numbers contradicting each other
(price $1–20 vs $2–20 vs the 2017 $2–5; float <10M vs <20M vs <30M).
Pre-reg #25 adjudicated the NEWER bands/caps on #16's frozen protocol
(`tools/measure_bandfloat.py`; #16's engine imported unchanged; #16 slot
reproduction matched to absdiff 0.0). H1/H2 on the hist universe, OOS
2022–2025 (the claims' own vintage); H3/H4 on the current universe, OOS
2016–2025, frozen 2026-08-13 float snapshot applied backward (survivorship
limitation documented in the pre-reg).

| Slot | Contrast | Verdict | Evidence |
|---|---|---|---|
| H1 | $2–20 vs >$20 | `tested, edge` — **EDGE** | +0.40pp (CI +0.30..+0.50, p<0.001), 993 paired dates, 431/618 names |
| H2 | $1–20 vs >$20 | `tested, edge` — **EDGE** | +0.52pp (CI +0.42..+0.61), incl. the non-empty lt2 tier (5,528 pairs, mean +6.26%!) |
| H3 | float ≤20M vs >20M | `tested, edge` — **EDGE** | +0.09pp (CI +0.04..+0.14, p 0.002), 2,504 dates, 65/534 names |
| H4 | float ≤10M vs >20M | `tested, edge` — **EDGE** | +0.25pp (CI +0.15..+0.34), 12 low-float names |

**Reading.** The drift verdict is surprising: his NEWER, WIDER bands do not
destroy the measured edge — the full sub-band table stays monotone (lt2
+6.26% / 2–5 +1.48% / 5–10 +0.50% / 10–20 +0.30% / >20 −0.11%), so every
band he has ever stated separates from the excluded >$20 zone. But the wide
bands are **diluted**: the $2–20 window earns +0.40pp where his 2017 $2–5
sweet spot earned +1.11pp (pre-reg #16 F1a) — the drift is real and it
costs ~2/3 of the edge, while the corpus's rhetoric escalated. The float
caps carry a real but much smaller edge (≤10M +0.25pp > ≤20M +0.09pp >
≤30M +0.11pp descriptive), monotone in the direction he claims. The
"2020 rule" conjunction ($2–20 AND float ≤20M) is the strongest cell
(+1.93pp, descriptive, n=106 dates — small-n caution). H3's per-year record
is NOT robust (negative 2017/2020) — the float edge is a level effect, not
...
## J.4 Selection follow-up verdicts — pre-registration #28 campaign (2026-09-02)

Daily-track follow-ups on the §J corpus's selection claims
(`tools/measure_selection2.py`; frozen before measurement; #16 engine
imported unchanged; same era/universe as #25). One Holm family of 4 slots;
floors 100 paired dates + 10 names/leg.

| Slot | Claim | Verdict | Evidence |
|---|---|---|---|
| H1 | rank-2/3 gainers beat rank-≥10 | `tested, inconclusive` — **INCONCLUSIVE** | 72/1,003 OOS days had ≥12 movers ≥+10% (the rare-mover population §J.3 C2 predicted); names 103/510 |
| H2 | ≥+10% names continue to +20% more than 0–2% names | `tested, no edge` — **NO EDGE** | +1.04pp (CI +0.06..+1.95, p 0.038) — claimed direction, not Holm-cleared (gate 0.0125) |
| H3 | just-below a half/whole dollar underperforms just-above | `tested, no edge` — **NO EDGE** | −0.04pp (CI −0.08..0.00, p 0.056) — direction consistent, CI touches zero; the corpus's most precise level-veto (ZS8-14) is nominally supported, never cleared |
| H4 | gap-and-go: ≥+2% gaps continue intraday | `tested, fade` — **FADE** | −0.22pp (CI −0.36..−0.06, p 0.008, Holm-cleared): big gappers' open→close return is LOWER than flat-open names' |

**Reading.** The first FADE of the §J campaigns: gap-and-go — a named,
flagship strategy — points the wrong way on daily bars: names gapping ≥+2%
mean-revert intraday relative to flat opens (an index-level effect: flat
opens drift up, gap-ups have already spent their move at the open — and his
own scans trade the gap intraday on 1-min bars, which this daily form does
not reproduce; the intraday form is the one to test on the #15 track). H2
and H3 both lean in his claimed direction with nominal p-values (0.038,
0.056) but fail Holm — the directional-whisper pattern now observed in four
consecutive campaigns (#8, #24, #28×2). H1's inconclusive is honest: the
